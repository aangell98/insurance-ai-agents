...
import { Phone, PhoneOff, Mic, MicOff, Loader2, CheckCircle2, AlertTriangle, Music } from 'lucide-react';
...
export default function VoiceCallModal({ open, onClose }: { open: boolean; onClose: () => void }) {
      ...
      <div className="w-full max-w-2xl overflow-hidden rounded-3xl bg-white shadow-2xl">
        ...
        <header className="flex items-center justify-between bg-gradient-to-r from-primary-600 to-primary-700 px-6 py-4 text-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold">Asistente de voz · Lola</p>
        ...
        <div className="flex h-[420px] flex-col">
          ...
          <div className="flex items-center justify-between border-t border-gray-200 bg-white px-6 py-4">
            ...
            <div className="text-xs text-gray-500">
              ...
              Cifrado · Azure OpenAI gpt-realtime-mini
            </div>
          </div>
        </div>
      </div>
...
function Bubble({ line }: { line: Line }) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      >
        {!isUser && <p className="mb-0.5 text-[10px] font-semibold uppercase tracking-wide text-primary-600">Lola</p>}
        {line.text}
      </div>
    </div>
  );
